...
export default defineComponent({
  setup() {
    ...
    const updateBackdropStyles = (show: boolean = false, transition: boolean, forHeight?: number) => {
    ...
    const show = () => {
      ...
      updateBackdropStyles(true, true, INITIAL_HEIGHT)
    };

    const close = () => {
      showSlide.value = false
      containerBodyScrollable.value = false
      updateBackdropStyles(true, true, 0)
    ...
    const afterTransitionLeave = (el: Element) => {
      updateBackdropStyles(false, false)
    ...
    const onTouchMove = (event: TouchEvent) => {
      ...
      if (newHeight <= MAX_HEIGHT) {
        ...
        updateBackdropStyles(true, false)
      }
    ...
    const onTouchEnd = (event: TouchEvent) => {
      ...
      if (deltaY > 25 && !isPanelExpanded.value) {
        containerRef.value!.classList.add('swipe-up-transition');

        const newHeight = MAX_HEIGHT + 'px';

        requestAnimationFrame(() => {
          containerRef.value!.style.height = newHeight;
          updateBackdropStyles(true, true, MAX_HEIGHT);
          containerBodyScrollable.value = true;
        });

        containerRef.value!.addEventListener('transitionend', function transitionEndHandler() {
          containerRef.value!.classList.remove('swipe-up-transition');
          containerRef.value!.removeEventListener('transitionend', transitionEndHandler);
